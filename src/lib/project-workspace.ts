...
function buildNextPage(input: ProjectBootstrapInput, backlogItems: GeneratedBacklogItem[]) {
  ...
  const adaptiveCollectionSection =
    isCommerce
      ? [
          ...
          '            <p className={`mt-3 max-w-xl text-sm leading-7 ${theme.mutedLabel}`}>제품군이 먼저 정리되면 가격과 스타일을 훨씬 빠르게 비교할 수 있습니다. 베스트, 신상품, 선물 추천 같은 관점으로 탐색을 시작하게 해주세요.</p>',
          "          </div>",
  ...
  const categorySpecialSection =
    isCommerce
      ? [
          '      <section id="collection-filters" className="mx-auto max-w-7xl px-6 py-10 lg:px-8">',
          '        <div className={`grid gap-6 lg:grid-cols-[0.98fr_1.02fr] ${theme.sectionShell}`}>',
          '          <div className={theme.sectionShell}>',
          '            <div className={`text-xs font-semibold uppercase tracking-[0.32em] ${theme.accentLabel}`}>컬렉션 필터</div>',
          '            <h2 className={`mt-2 text-3xl font-semibold tracking-tight ${theme.pageShell.includes("text-slate-50") ? "text-white" : "text-slate-950"}`}>취향과 구매 목적에 맞게 컬렉션을 빠르게 좁힙니다</h2>',
          '            <p className={`mt-4 max-w-2xl text-sm leading-7 ${theme.mutedLabel}`}>커머스 첫 화면에서는 탐색 기준이 먼저 보여야 합니다. 신상품, 베스트, 선물 추천처럼 바로 고를 수 있는 필터를 전면에 두면 이탈이 줄어듭니다.</p>',
          ...
          "            {blueprint.collectionFilters.slice(0, 4).map((item, index) => (",
          '              <article key={item.label} className={theme.cardShell}>',
          '                <div className={`text-xs font-semibold uppercase tracking-[0.24em] ${theme.accentLabel}`}>필터 0{index + 1}</div>',
          '                <h3 className={`mt-2 text-lg font-semibold ${theme.pageShell.includes("text-slate-50") ? "text-white" : "text-slate-950"}`}>{item.label}</h3>',
          '                <p className={`mt-2 text-sm leading-6 ${theme.mutedLabel}`}>{item.note}</p>',
          "              </article>",
          "            ))}",
          "          </div>",
          "        </div>",
  ...
  return [
    ...
    ...(blueprint.category === "commerce"
      ? [
          ...
          '            <p className={`mt-4 max-w-2xl text-sm leading-7 ${theme.mutedLabel}`}>제품군이 달라도 구매 전 확인해야 할 기준은 비슷합니다. 가격, 소재, 옵션, 배송, 교환 기준을 먼저 읽게 하면 문의와 결제가 훨씬 자연스러워집니다.</p>',
          ...
          '          <div className={theme.sectionShell}>',
          '            <div className={`text-xs font-semibold uppercase tracking-[0.32em] ${theme.accentLabel}`}>구매 안내</div>',
          '            <h3 className={`mt-2 text-2xl font-semibold tracking-tight ${theme.pageShell.includes("text-slate-50") ? "text-white" : "text-slate-950"}`}>배송, 교환, 비교 포인트를 먼저 확인하세요</h3>',
          '            <div className="mt-6 grid gap-4 sm:grid-cols-2">',
          "              {blueprint.serviceCards.map((item) => (",
          '                <article key={item.title} className={theme.cardShell}>',
